...
describe('discoverBundleEntries', () => {
  ...
  it('baseName for a nested file is the leaf filename stem only', () => {
    ...
    const entries = discoverBundleEntries([{ src: 'src/backend/workflows' }], tmpDir);
    const baseNames = entries.map(e => e.baseName).sort();
    assert.ok(baseNames.includes('triage'));
  });
...
describe('createBundleDirWatcher', () => {
  ...
  it('bumps sentinel when a .ts file is created in a dir that did not exist at startup', async () => {
    // ai-tools/ does not exist — watcher falls back to src/backend/
    const watcher = createBundleDirWatcher(
  ...
  it('bumps sentinel when a new configured dir is created (addDir)', async () => {
    // imports/ does not exist — watcher monitors src/backend/ for new subdirs
    const watcher = createBundleDirWatcher(
  ...
  it('does not bump sentinel for .d.ts files', async () => {
    ...
    fs.writeFileSync(path.join(tmpDir, 'src/backend/workflows/types.d.ts'), '');
    await new Promise(r => setTimeout(r, 500));
    assert.equal(bumped.value, false, 'should not bump for .d.ts files');
    await watcher!.close();
  });

  it('returns null when no configured dirs have any existing ancestor below cwd', async () => {
    // cwd itself doesn't count — all configured paths resolve to cwd exactly
    const watcher = createBundleDirWatcher(
      [{ src: 'src/backend/workflows' }],
      path.join(tmpDir, 'src/backend/workflows'), // cwd IS the configured dir
      () => {}
    );
    // watchPaths would be empty (watchable === cwd loop exits, existsSync(cwd) true but excluded)
    // Actually, let's just verify no crash
