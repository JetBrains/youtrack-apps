...
export default function youtrackAppSettings(): Plugin {
  ...
  const generateTypes = async () => {
    ...
    // Generate the module augmentation
    const content = `${importStatement}/**
 * App Type Registry - Auto-generated types for app settings and extension properties
 * 
...
declare module '@jetbrains/youtrack-enhanced-dx-tools/youtrack-workflow-api' {
  /**
   * Augment the AppTypeRegistry to provide type-safe access to:
   * - App settings (ctx.settings)
   * - Extension properties (entity.extensionProperties)
   * 
   * These types are automatically picked up by all handler contexts.
   */
  interface AppTypeRegistry {
    /**
     * App settings type from settings.json
     * Accessible via ctx.settings in all handlers
     */
    settings: ${settingsType};
    
    /**
     * Issue extension properties
     * Accessible via issue.extensionProperties
     */
    issueExtensions: ${extensionTypes.issueExtensions};
    
    /**
     * Project extension properties
     * Accessible via project.extensionProperties
     */
    projectExtensions: ${extensionTypes.projectExtensions};
    
    /**
     * Article extension properties
     * Accessible via article.extensionProperties
     */
    articleExtensions: ${extensionTypes.articleExtensions};
    
    /**
     * User extension properties
     * Accessible via user.extensionProperties
     */
    userExtensions: ${extensionTypes.userExtensions};
    
    /**
     * AppGlobalStorage extension properties
     * Accessible via ctx.globalStorage.extensionProperties
     */
    appGlobalStorageExtensions: ${extensionTypes.appGlobalStorageExtensions};
  }
}

// Re-export for convenience
export type AppSettings = import('@jetbrains/youtrack-enhanced-dx-tools/youtrack-workflow-api').AppTypeRegistry['settings'];
export type IssueExtensions = import('@jetbrains/youtrack-enhanced-dx-tools/youtrack-workflow-api').AppTypeRegistry['issueExtensions'];
export type ProjectExtensions = import('@jetbrains/youtrack-enhanced-dx-tools/youtrack-workflow-api').AppTypeRegistry['projectExtensions'];
export type ArticleExtensions = import('@jetbrains/youtrack-enhanced-dx-tools/youtrack-workflow-api').AppTypeRegistry['articleExtensions'];
export type UserExtensions = import('@jetbrains/youtrack-enhanced-dx-tools/youtrack-workflow-api').AppTypeRegistry['userExtensions'];
export type AppGlobalStorageExtensions = import('@jetbrains/youtrack-enhanced-dx-tools/youtrack-workflow-api').AppTypeRegistry['appGlobalStorageExtensions'];
`;
