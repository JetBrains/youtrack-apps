 ...
 * Extends BaseArticle to inherit base article properties.
 */
...
export interface IssueAttachment extends PersistentFile {
  ...
  readonly content?: unknown; // InputStream type - byte stream representation
  readonly author?: User;
  readonly created?: number;
...
export interface User {
  ...
  readonly attributes?: Record<string, unknown>; // UserAttributes - user-specific attributes
 ...
 * To retrieve a value that is stored in an attribute for a referenced work item, use the attribute name as the key.
 */
export interface WorkItemAttributes {
  readonly [key: string]: WorkItemAttributeValue | unknown;
}
