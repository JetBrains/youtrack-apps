...
/**
 * YouTrack entity types that should be imported from @jetbrains/youtrack-workflow-types
 */
...
export const generateExtendedEntities = (extensions: EntityExtensions): string => {
  ...
  const importStatements = imports.size > 0
    ? `import type { ${Array.from(imports).sort().join(', ')} } from '@jetbrains/youtrack-workflow-types/workflowTypeScriptStubs';\n\n`
    : '';
...
const generateContextUtilities = (extensions: EntityExtensions): string => {
  ...
  if (entitiesToExtend.size === 0) {
    return `// No extended entities - use base context types from @jetbrains/youtrack-apps-tools/dx
export type ExtendedIssueCtx<T extends import('@jetbrains/youtrack-apps-tools/dx').IssueCtx> = T;
export type ExtendedProjectCtx<T extends import('@jetbrains/youtrack-apps-tools/dx').ProjectCtx> = T;
export type ExtendedArticleCtx<T extends import('@jetbrains/youtrack-apps-tools/dx').ArticleCtx> = T;
export type ExtendedUserCtx<T extends import('@jetbrains/youtrack-apps-tools/dx').UserCtx> = T;
`;
  }
