...
export function resolveAppName(appDir?: string): undefined | string {
  if (!appDir) {
    exit(new Error(i18n('App directory should be defined')), ExitCode.Usage);
    return;
  }
  ...
  if (!fs.existsSync(appPath)) {
    exit(new Error(i18n("App directory doesn't exist")), ExitCode.Usage);
    return;
  }
  ...
  if (!appName) {
    exit(new Error(i18n("App doesn't contain manifest.json file")), ExitCode.Usage);
  }
