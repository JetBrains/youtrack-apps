  ...
  const Configuration = useCallback(
    () => (
      <Config
        ...
        activeEditTab={editTabId}
      />
    ),
    [config.title, config.tabs, config.refreshPeriod, isLoading, onConfigSubmit, onConfigCancel, editTabId],
  );

  const widgetContent = useCallback(
    () => <Content key={refreshKey} tabs={config.tabs} editable onTabEdit={onTabEdit}/>,
    [refreshKey, config.tabs, onTabEdit],
  );
